...
def play_video_with_fps(file_path):
    ...
    while vid.isOpened():
        ...
        if not ret:
            break
        stframe.image(frame, channels="BGR", use_container_width=True)  # Perbaikan parameter
        time.sleep(1 / fps)
    vid.release()
...
def predict_image(file_path, confidence):
    model = YOLO('SGD x 640 x 0.0005 x 80 15 5.pt')
...
def main():
    ...
    if uploaded_file:
        ...
        if file_type in ['mp4', 'avi']:
            ...
            st.image(temp_file_path, caption='Uploaded Image', use_container_width=True)  # Perbaikan parameter
            if st.button('Predict'):
                no_accident_count, moderate_count, severe_count, annotated_image_path = predict_image(temp_file_path, confidence)
                st.write(f"No accident detected: {no_accident_count}")
                st.write(f"Moderate accidents detected: {moderate_count}")
                st.write(f"Severe accidents detected: {severe_count}")
                st.image(annotated_image_path, caption="Prediction Output", use_container_width=True)  # Perbaikan parameter
